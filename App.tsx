...
export default function App() {
  ...
  function selectIds(ids: string[]) { if (ids.length === 0) return; setSelectedIds((prev) => { const next = new Set(prev); ids.forEach((id) => next.add(id)); return next; }); }
  ...
  async function enqueueDownloadBatch(sounds: SendableSound[], options: DownloadQueueOptions = {}): Promise<string> {
    ...
    try {
      ...
      const batch: DownloadBatch = {
        ...
        sounds,
        state: options.deferStart ? "queued" : "queued",
  ...
  function clearCompletedDownloads() { setDownloadQueue((prev) => prev.filter((batch) => batch.state !== "done")); }
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      } finally { activeBatchRef.current = null; }
    })();
  }, [downloadQueue]);
  ...
  return (
    <div className="app">
      ...
      <FramePanel apiKeys={apiKeys} onDesignGenerated={replaceLayers} onSceneNameChange={setSceneName} />
