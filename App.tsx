    ...
    <div className="app">
      ...
      <div className="app__grid" style={{ gridTemplateColumns: "repeat(3, minmax(0, 1fr))" }}>
        {ELEMENTS.map(({ id, label, hint }) => (
          <SoundtrackPanel key={id} elementId={id} label={label} hint={hint} layers={layers[id]}
            onLayersChange={(next) => setLayers((prev) => ({ ...prev, [id]: next }))}
            apiKeys={apiKeys} selectedIds={selectedIds} onToggleSelect={toggleSelect} onSelectIds={selectIds}
            onSetCategorySelection={(selectAll) => setCategorySelection(id, selectAll)} globalSoloActive={globalSoloActive} />
        ))}
      </div>
    </div>
  );
