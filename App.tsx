...
export default function App() {
  ...
  useEffect(() => { void getDownloadRootStatus().then((root) => setDownloadRootName(root?.name ?? "")).catch(() => {}); }, []);
  ...
  function handleNewScene() { sceneGroupRef.current = null; sceneGroupCreationRef.current = null; }
  async function handleChangeDownloadRoot() {
    if (changingDownloadRoot) return;
    setChangingDownloadRoot(true); setDownloadRootError(null);
    try { const root = await changeDownloadRoot(); setDownloadRootName(root.name); sceneGroupRef.current = null; sceneGroupCreationRef.current = null; }
    catch (error) { setDownloadRootError(error instanceof Error ? error.message : "No se pudo cambiar la carpeta raíz."); }
    finally { setChangingDownloadRoot(false); }
  }
  ...
  function setCategorySelection(elementId: SoundtrackElement, selectAll: boolean) {
    ...
    setSelectedIds((prev) => { const next = new Set(prev); ids.forEach((id) => (selectAll ? next.add(id) : next.delete(id))); return next; });
  }
  async function ensureSceneGroup(): Promise<{ rootName: string; groupName: string }> {
    const root = await getOrChooseDownloadRoot(); setDownloadRootName(root.name);
    if (sceneGroupRef.current) return { rootName: root.name, groupName: sceneGroupRef.current };
    if (!sceneGroupCreationRef.current) {
      sceneGroupCreationRef.current = (async () => { const group = await createSceneGroupDirectory(null, sceneName); sceneGroupRef.current = group.name; return { rootName: root.name, groupName: group.name }; })().finally(() => { sceneGroupCreationRef.current = null; });
    }
  ...
  async function enqueueDownloadBatch(sounds: SendableSound[], onCompleted?: BatchCompletion): Promise<string> {
    ...
    setDownloadQueue((prev) => [...prev, batch]); return batchId;
  }
  ...
  function markQueuedBatchCancelled(id: string) {
    ...
    batchCompletionRef.current.delete(id); downloadControllerRef.current.delete(id);
  }
  function cancelDownloadBatch(id: string) { const batch = downloadQueue.find((candidate) => candidate.id === id); const controller = downloadControllerRef.current.get(id); if (!batch || !controller) return; controller.cancelBatch(); if (batch.state === "queued") markQueuedBatchCancelled(id); }
  function cancelDownloadItem(batchId: string, itemId: string) {
    const batch = downloadQueue.find((candidate) => candidate.id === batchId); const controller = downloadControllerRef.current.get(batchId); if (!batch || !controller) return; controller.cancelItem(itemId);
    if (batch.state === "queued") {
      updateBatch(batchId, (current) => { const items = current.progress.items.map((item) => item.id === itemId ? { ...item, state: "cancelled" as const, speedBytesPerSecond: 0, error: undefined } : item); const allCancelled = items.every((item) => item.state === "cancelled"); return { ...current, state: allCancelled ? "cancelled" : current.state, error: allCancelled ? "Grupo cancelado porque todos sus archivos fueron cancelados." : current.error, progress: { ...current.progress, current: allCancelled ? current.progress.total : current.progress.current, cancelled: items.filter((item) => item.state === "cancelled").length, items } }; });
  ...
  async function importBatchToReaper(batchId: string) {
    ...
    if (!result.ok) updateBatch(batchId, (current) => ({ ...current, error: result.error ?? "No se pudo preparar la importación a REAPER." }));
  }
  useEffect(() => {
    if (activeBatchRef.current) return;
    const nextBatch = downloadQueue.find((batch) => batch.state === "queued"); if (!nextBatch) return;
    const controller = downloadControllerRef.current.get(nextBatch.id) ?? createDownloadController(); downloadControllerRef.current.set(nextBatch.id, controller); activeBatchRef.current = nextBatch.id; updateBatch(nextBatch.id, (batch) => ({ ...batch, state: "downloading" }));
    void (async () => {
      try {
        ...
        const completedSuccessfully = result.failed === 0 && result.cancelled === 0; const wasCancelled = result.cancelled > 0;
        ...
        if (completion && completedSuccessfully) { try { await completion(); } finally { batchCompletionRef.current.delete(nextBatch.id); } } else batchCompletionRef.current.delete(nextBatch.id);
      } catch (error) { batchCompletionRef.current.delete(nextBatch.id); updateBatch(nextBatch.id, (batch) => ({ ...batch, state: "error", error: error instanceof Error ? error.message : "No se pudo completar la descarga." })); }
      finally { downloadControllerRef.current.delete(nextBatch.id); activeBatchRef.current = null; }
    })();
  }, [downloadQueue]);
  ...
  return (
    <div className="app">
      ...
      <div className="frame-storage" aria-live="polite"><div className="frame-storage__main"><div className="frame-storage__label">Carpeta de almacenamiento</div><div className="frame-storage__path" title={downloadRootName || "No seleccionada"}>{downloadRootName || "No seleccionada"}</div><div className="frame-storage__help">Los grupos y archivos nuevos se guardarán aquí.</div>{downloadRootError && <div className="frame-storage__error">{downloadRootError}</div>}</div><button type="button" className="frame-storage__change" onClick={() => void handleChangeDownloadRoot()} disabled={changingDownloadRoot}>{changingDownloadRoot ? "Seleccionando…" : "Cambiar carpeta"}</button></div>
