...
const CERTAINTY_LABEL: Record<Certainty, string> = { observed: "observado", probable: "probable", possible: "posible" };

function Cue({ cue }: { cue: SoundCue }) {
  return <span className={`scene-analysis__cue certainty-${cue.certainty}`}>{cue.text}<em>{CERTAINTY_LABEL[cue.certainty]}</em></span>;
}
function CueRow({ title, cues }: { title: string; cues: SoundCue[] }) {
  if (cues.length === 0) return null;
  return <div className="scene-analysis__section"><div className="scene-analysis__section-title">{title}</div><div className="scene-analysis__cues">{cues.map((c, i) => <Cue key={`${title}-${i}`} cue={c} />)}</div></div>;
}
function SingleCueRow({ title, cue }: { title: string; cue: SoundCue }) {
  if (!cue.text) return null;
  return <div className="scene-analysis__section"><div className="scene-analysis__section-title">{title}</div><div className="scene-analysis__cues"><Cue cue={cue} /></div></div>;
}
